...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      phone: '', department: user.department || '', year: '',
      role: user.role || 'member', teamId: user.teamId || null,
...
export async function PUT(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({
      _id: user._id, id: user._id,
      name: body?.name || user.name || 'User', email: user.email || '',
      registrationNumber: user.registrationNumber || '',
      phone: body?.phone || '', department: body?.department || user.department || '',
      year: body?.year || '', role: user.role || 'member',
      teamId: user.teamId || null, internetCredentialId: user.internetCredentialId || null,
      verified: false, createdAt: new Date().toISOString(),
    });
  }
}
