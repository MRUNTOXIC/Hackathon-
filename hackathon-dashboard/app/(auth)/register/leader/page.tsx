...
export default function RegisterLeaderPage() {
  ...
  } = useForm<RegisterData>({ resolver: zodResolver(registerSchema), defaultValues: { email: pendingEmail } });

  const onRegisterSubmit = async (data: RegisterData) => {
    try {
      setError('');
      await registerUser('/auth/register-leader', {
    ...
    <div className="max-w-lg mx-auto">
      <div className="text-center mb-8 animate-fadein">
        ...
        <h1 className="text-2xl font-bold text-white">
          {step === 'form' ? 'Complete Registration' : 'Register Team'}
        </h1>
        <p className="text-slate-400 text-sm mt-1">
          ...
          {step === 'form' && 'Create your team and become the leader'}
        </p>
      </div>
